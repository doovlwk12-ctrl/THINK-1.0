...
2. **تأمين الـ Middleware:** إذا كان الخطأ يحدث **قبل** وصول الطلب إلى مسار الرسائل (مثلاً فشل `getSupabaseSession` أو أي استثناء في الـ middleware)، يتم اعتراض الاستثناء وإرجاع **503** لطلبات `/api/*` بدلاً من 500، حتى لا يظهر للمستخدم "Internal Server Error".
3. **استخراج آمن لـ `orderId`:** دعم `context.params` كـ **Promise** (Next.js 15) حتى لا يبقى `orderId` غير معرّف ويسبب سلوكاً غير متوقع.
4. **تسجيل تفصيلي في السيرفر:** طباعة `message`, `code`, `meta`, `name`, و **stack** في الـ catch حتى يمكن تشخيص السبب الحقيقي من سجلات Vercel.
5. **الواجهة الأمامية:** إيقاف الـ polling عند رد **500** أو **503** وعرض رسالة موحدة ("تعذر الاتصال بالخادم. تحقق من الاتصال وأعد المحاولة.") مع زر إعادة المحاولة.
...
- [ ] **Middleware:** في `middleware.ts` يتم استدعاء `getSupabaseSession` داخل **try/catch**؛ عند أي استثناء لطلبات `/api/*` يُرجَع **503** ("تعذر التحقق من الجلسة. أعد المحاولة لاحقاً."). كما أن الدالة الافتراضية للـ middleware مُغلّفة بـ try/catch لتفادي 500 من أي خطأ غير متوقع.
...
- [ ] **متغيرات البيئة:** على Vercel مضبوط `DATABASE_URL` بشكل صحيح (ووضع Transaction مع `?pgbouncer=true` إن لزم). عند استخدام Supabase Auth: `NEXT_PUBLIC_SUPABASE_URL` و `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
