...
4. **اسم المشروع (Project Name):** يجب أن يكون **أحرفاً صغيرة فقط**، مع أرقام وشرطة `-` فقط (بدون نقطة أو شرطات متتالية `---`). مثال صحيح: `think-1-0` أو `fekra-platform`. لا تستخدم مثلاً `THINK-1.0` أو `doovlwk12-ctrl-THINK-1.0` لأنها تحتوي أحرفاً كبيرة أو نقطة.
5. اختر **Framework Preset**: Next.js (يُكتشف تلقائياً عادة).
6. **Root Directory** اتركه `./` إلا إذا المشروع داخل مجلد فرعي.
7. **لا تضغط Deploy بعد.** انتقل أولاً إلى **Environment Variables** (أو اضغط Deploy ثم نضيف المتغيرات ونعيد النشر).
