...
- **على Vercel (أو استضافة بدون قرص ثابت):** التخزين المحلي غير متوفر. يجب إعداد **Supabase Storage** حتى تظهر صور المخططات:
  - `NEXT_PUBLIC_SUPABASE_URL` و `SUPABASE_SERVICE_ROLE_KEY`
  - bucket باسم **`orders`** في Supabase (يمكن أن يكون عاماً أو خاصاً؛ مسار `/api/orders/[id]/plans/[planId]/image` يجلب الصورة عبر Service Role).
- إذا كانت الملفات قد رُفعت محلياً (`/uploads/plans/`) ثم نُشر التطبيق، الملفات غير موجودة على السيرفر → **الصور لن تظهر**؛ الحل: إعداد Supabase ورفع المخططات من جديد.
