...
| الحل | الوصف |
|------|--------|
| **PlanImage** | مكوّن يعرض الروابط الخارجية بـ `<img>` ويمرر روابط Supabase عبر `/api/image-proxy` |
| **images.unoptimized** | في `next.config.js` لضمان عدم مرور أي صورة عبر `_next/image` |
| **/api/image-proxy** | مسار يجلب الصورة من Supabase ويعيدها للمتصفح (تجنب 400 و CORS) |
| **تحميل المخطط** | عند فشل الـ proxy يعيد التوجيه 302 لرابط الملف على Supabase |
...
- تعيين **NEXT_PUBLIC_SUPABASE_URL** في متغيرات بيئة Vercel (مطلوب لعمل `/api/image-proxy`).
