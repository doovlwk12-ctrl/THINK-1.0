...
**تحذيرات بناء Vercel (npm deprecated، webpack):** إن ظهرت تحذيرات أثناء `npm install` أو `next build` (مثل rimraf، glob، eslint، google-p12-pem)، البناء ينجح رغمها. لخطة تفصيلية لحلها أو تقليلها راجع [docs/VERCEL-BUILD-WARNINGS-PLAN.md](VERCEL-BUILD-WARNINGS-PLAN.md).
