...
/** استخراج bucket و path من أي رابط Supabase Storage (مثل مسار الصورة الموحّد). */
...
export async function GET(
  ...
  try {
    ...
    if (plan.purgedAt || !plan.fileUrl?.trim()) {
      return new Response(
        JSON.stringify({ error: `تم حذف الملف من الأرشيف بعد ${ARCHIVE_PURGE_DAYS_AFTER_DEADLINE} يوماً من الموعد النهائي.` }),
    ...
    // Supabase: تحميل عبر Service Role (يعمل مع bucket خاص)، مع فك ضغط .gz عند الحاجة
